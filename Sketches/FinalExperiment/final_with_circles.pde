...
if ((frameCount % 20) == 1&& world.getBodies().size()<=(200*s+1)) {
  ...
  **/
  ...
  if(userList.length>0){
    l.vertex(0,height/2);
    head = drawSkeleton(userList[0],0);
  }
  head.x = map(head.x,0,width,0,255);
...
if(world.getBodies().size()>=(200*s+1)){
 setup(); 
}
...
PVector drawSkeleton(int userId, int i)
{
  ...
  jP.y = map(jP.y,0,480,height/3,height);
  println(jP.y);
  return(jP);
}
